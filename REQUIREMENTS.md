...
여행 사진에 내장된 EXIF GPS 메타데이터를 자동으로 추출하고, Vision AI로 사진을 분류한 뒤, 3D 지구본 위에 핀으로 시각화하는 웹 서비스를 개발한다.
...
| 3D 지구본 | Globe.gl | CDN, Three.js 기반 |
...
| ID | 요구사항 |
|----|---------|
| F-04-1 | Globe.gl로 3D 지구본을 렌더링한다. (Blue Marble 텍스처 사용) |
| F-04-2 | 사진 업로드 완료 시 해당 GPS 좌표에 색상 핀을 배치한다. |
| F-04-3 | 핀 색상은 첫 번째 태그 카테고리에 따라 다르게 표시된다. |
| F-04-4 | 새 핀 추가 시 지구본이 해당 위치로 자동 이동(flyTo)한다. |
| F-04-5 | 지구본은 기본적으로 자동 회전하며, 핀 추가 시 자동 회전이 일시 정지된다. |
...
| F-06-4 | 사이드바 항목 클릭 시 지구본이 해당 핀 위치로 이동하고 팝업이 열린다. |
...
| ID | 요구사항 |
|----|---------|
| NF-07 | Nominatim API 이용 약관 준수: 초당 1요청 제한, User-Agent 헤더 포함 |
| NF-08 | Ollama는 로컬에서 실행되어야 하며 외부 인터넷 연결이 필요 없다. |
| NF-09 | 모든 라이브러리는 무료·오픈소스로 제한한다. |
