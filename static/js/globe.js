...
async function activateGlobalMap(el) {
  const config = await loadDetailMapConfig();
  if (!config?.enabled || !config.styleUrl) {
    if (typeof toast === 'function') toast('Set PINDROP_MAPTILER_KEY to enable the global detail map.', 'error');
    return false;
  }
  ...
  try {
    ...
    if (typeof toast === 'function') toast('Global detail map failed to load.', 'error');
    return false;
  }
...
async function toggleGlobalMapMode() {
  ...
  if (!el) return false;
  return mapMode === 'global' ? deactivateGlobalMap(el) : activateGlobalMap(el);
}
...
function initGlobe(containerId) {
  ...
  const surface = createKoreaMapSurface(el);
  ...
  globeInstance = {
    ...
    arcsData(data) {
      ...
      renderKoreaRoutes(el, data || []);
      renderGlobalRoutes(data || []);
      return globeInstance;
    },
    htmlElementsData(data) {
      surface.dataset.htmlElementCount = String((data || []).length);
      return globeInstance;
    },
  };
